...
# 定义一个保存图像的函数
def save_figure(adata, genes, save_path, filename, plot_type,result_dir=None, dpi=300, Finally=False):
    ...
    if plot_type == 'umap':
        fig = sc.pl.umap(
            ...
            frameon=False,
            ncols=3,
            size=5, return_fig=True
        )
    elif plot_type == 'dotplot':
        # 绘制 dotplot 图形
        sc.pl.dotplot(adata, genes, groupby="leiden", return_fig=True)
        fig = plt.gcf()
        ...
        command = [
            "/home/xjh/anaconda3/envs/lab-gene/bin/python", script_path,
            "--csv", csv_file_path,
            "--outdir", save_dir,
            "--cmap", 'viridis',
        ...
        tif_file_path = os.path.join(save_dir, f"{genes}_with_legend.tif")
        jpg_file_path = os.path.join(save_dir, f"{genes}_with_legend.jpg")
...
if os.path.exists(gene_id_file_path):
    ...
    for idx, gene_id in enumerate(gene_ids):
        ...
        save_figure(sc_test, gene_id, save_path, f'spatial_dotplot_gene_projection_{gene_id}', plot_type='dotplot')
        save_figure(sc_test, gene_id, save_path, f'spatial_spatial_gene_projection_{gene_id}', plot_type='spatial')
        save_figure(sc_test, gene_id, save_path,result_dir=result_dir, filename='clustered_data', plot_type='cluster', Finally=is_last)
